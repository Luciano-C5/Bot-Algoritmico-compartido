...
# Formato: "seccion.atributo" → (objeto_seccion, nombre_atributo, tipo_python)
_PARAM_MAP = {
    # Capital
...
class Config:
    ...
    def apply_change(
        ...
        # Aplicar en memoria (siempre, independiente del modo)
    ...
    def _resolve_section(self, section_path: str) -> Any:
        """
        Resuelve el objeto de sección a partir del path.
        Ej: "modes.scalp" → self.modes.scalp
        """
    ...
    def _save_override(self, param: str, value: Any) -> None:
        """
        Guarda o actualiza un parámetro en config_override.json.
        El archivo es un dict plano de "seccion.atributo" → valor.
        """
    ...
    def _write_to_source(
        ...
        """
        Modifica el valor del parámetro directamente en config.py.
        Busca la línea con el atributo dentro de la sección correcta
        y reemplaza el valor.

        Devuelve True si tuvo éxito, False si no encontró la línea.
        """
        ...
        try:
            ...
            # Armar el patrón de búsqueda: "    attr: tipo = valor_anterior"
            # Acepta cualquier valor anterior (número, string, bool)
            ...
            # Patrón que matchea la línea de definición del atributo
    ...
    # ─────────────────────────────────────────
    # CARGA DE OVERRIDES AL ARRANCAR
    # ─────────────────────────────────────────

    def load_overrides(self, path: str = "config_override.json") -> None:
        """
        Carga todos los overrides guardados desde sesiones anteriores.
        Se llama automáticamente al crear la instancia cfg.
        """
    ...
    def clear_overrides(self, path: str = "config_override.json") -> str:
        """
        Borra todos los overrides persistentes.
        Los valores vuelven a los defaults de config.py.
        Útil desde Flask: botón 'Restaurar valores originales'.
        """
    ...
    def params_list(self) -> list[dict]:
        """
        Lista todos los parámetros modificables con su valor actual.
        La interfaz Flask usa esto para generar los controles de la UI.
        """
        result = []
...
if __name__ == "__main__":
    ...
    assert cfg.leverage.get_leverage(2) == 3
    assert cfg.leverage.get_leverage(3) == 2
    ...
    print("\n── Test modo SESSION ──")
    original = cfg.threshold.level_1
    msg = cfg.apply_change("threshold.level_1", 0.60, mode="session")
    print(f"  {msg}")
    assert cfg.threshold.level_1 == 0.60
    assert not os.path.exists("config_override.json")
    cfg.threshold.level_1 = original  # restaurar
    print("  Session: no crea archivo ✓")

    print("\n── Test modo PERSISTENT ──")
    msg = cfg.apply_change("capital.initial_capital", 250.0, mode="persistent")
    print(f"  {msg}")
    assert cfg.capital.initial_capital == 250.0
    assert os.path.exists("config_override.json")
    # Verificar que un nuevo Config lo carga
    cfg2 = Config()
    cfg2.load_overrides()
    assert cfg2.capital.initial_capital == 250.0
    print("  Persistent: nuevo Config carga el override ✓")
    # Limpiar
    msg = cfg.clear_overrides()
    print(f"  {msg}")
    cfg.capital.initial_capital = 100.0

    print("\n── Test params_list ──")
    params = cfg.params_list()
    assert len(params) > 0
    assert all("param" in p and "value" in p and "type" in p for p in params)
    print(f"  {len(params)} parámetros listados ✓")

    print("\n── Test modo PERMANENT ──")
    # Hacemos copia del archivo antes de modificarlo
    src = os.path.abspath(__file__)
    backup = src + ".bak"
    shutil.copy2(src, backup)
    try:
        original_val = cfg.threshold.level_2
        msg = cfg.apply_change("threshold.level_2", 0.47, mode="permanent")
        print(f"  {msg}")
        # Verificar que el archivo cambió
        with open(src) as f:
            content = f.read()
        assert "0.47" in content
        print("  Permanent: config.py modificado ✓")
    finally:
        # Restaurar el archivo original
        shutil.copy2(backup, src)
        os.remove(backup)
        cfg.threshold.level_2 = original_val
        print("  config.py restaurado desde backup ✓")
